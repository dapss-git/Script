 ...
 *-(Server)-:* ${ox?.headers?.server}`;
        ...
        if (content.includes("mpeg") || content.includes("m4a")) {
            ...
            // --- Tambahan: Kirim isi teks ---
            m.reply(ox.body.slice(0, 10000)); // Limit 10rb karakter biar gak lag

        } else if (/json/.test(content)) {
            const jsres = JSON.stringify(ox.json, null, 2);
            m.reply(jsres);

        } else if (/application|text/.test(content)) { // Ditambah 'text' biar support file txt/css dll
            await conn.sendMessage(m.chat, {
                document: ox.buffer,
                mimetype: content,
                fileName: "result." + (content.split("/")[1] || "bin"),
                caption: "✅ Nih Get Type: Document!"
            }, { quoted: m });
            // --- Tambahan: Kirim isi teks ---
            m.reply(ox.body.slice(0, 10000));
