...
let Izumi = async (m, { conn, text }) => {
  ...
  if (/image/.test(mime)) {
    ...
    return conn.reply(m.chat, '✅ Dah UpStatus Nya Tengok Di Reply', sta);
  } else if (textInput && !warna) { 
    // TAMBAHAN: Support status text dengan warna random
    const listWarnaWA = ['#000000', '#25D366', '#34B7F1', '#FFD700', '#FF8C00', '#FF3B30', '#9C27B0', '#00BCD4', '#5F7C8A'];
    const randomColor = listWarnaWA[Math.floor(Math.random() * listWarnaWA.length)];

    const sta = await groupStatus(conn, id, {
      text: textInput,
      backgroundColor: randomColor
    });
    return conn.reply(m.chat, '✅ Dah UpStatus Text (Warna Random) Nya Tengok Di Reply', sta);
  } else if (warna) {
