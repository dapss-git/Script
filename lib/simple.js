...
const baill = await import("baileys")
...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    */
     ...
     */
        ...
        getName: {
            value: async function(id = "", withoutContact = false) {
                ...
                if (id.endsWith("@g.us")) {
                    return this
                        ...
                        .then(
                            ...
                            metadata.name ||
                            metadata.subject ||
                            PhoneNumber("+" + Number(parseInt(id))).getNumber(
                                "international",
                            ),
                        );
                }
                ...
                v =
                    ...
                    areJidsSameUser(id, this.user.id) ?
                    this.user :
                    (this?.chats?.[id] || null);
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {
                    ...
                    sender: {
                        get() {
                            ...
                            conn._lidCache = conn._lidCache || new Map();

                            const jid = conn?.decodeJid ?
                                conn.decodeJid(raw) :
                                typeof raw?.decodeJid === "function" ?
                                raw.decodeJid() :
                                raw;

                            if (conn._lidCache.has(jid)) {
                                return conn._lidCache.get(jid);
                            }
        ...
        reply: {
            value(text, chatId, options) {
                ...
                const jidTask = conn.signalRepository.lidMapping
                    .getPNForLID(m.sender)
                    .then(v => v?.replace(/:\d+@/, '@'))
                    .catch(() => null)
                    .then(v => v || m.sender)

                const ppTask = conn.profilePictureUrl(m.sender, 'image')
                    .catch(() => "https://c.top4top.io/p_36459rzan1.jpg")

                return Promise.all([jidTask, ppTask]).then(([jid, ppUrl]) => {
